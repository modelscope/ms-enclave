...
必须实现/覆写：
- `required_sandbox_types`：声明该工具可运行的沙箱类型列表（返回 `None` 表示所有类型均可）。
...
- 工具与沙箱兼容性：`Tool.is_compatible_with_sandbox` 会根据 `required_sandbox_types` 与 `SandboxType.is_compatible` 判定。
...
class TimeTellerTool(Tool):
    ...
    @property
    def required_sandbox_types(self) -> Optional[List[SandboxType]]:
        # 声明为 DOCKER => 在 DOCKER、DOCKER_NOTEBOOK 等兼容类型中可用
...
- 兼容性：
  - 工具应通过 `required_sandbox_types` 明确要求；若无要求，返回 `None`。
